...
namespace partymode
{
    ...
    {
        Timer rewardTimer;
        private int allowSeekerMovementCounter=15;
        private Player seeker=null;
        public HideSeekPlayMode() :
            base(
                "hideseek",
                new CustomSpectator(new Vector3(-1158.2, -1180.6, 129.5), 60, 10000),
                "Tryb rozgrywki: ~g~Hide&Seek(chowany) ~w~~n~Za chwile wystapi wylosowanie osoby szukajacej.~n~Jezeli nia nie zostaniesz, po rozpoczeciu~n~"+
                "rozgrywki ukryj sie jak najszybciej.~n~ Osoba szukajaca bedzie miala bron, podczas gdy Ty~n~ pozostaniesz bezbronny. Nie odrodzisz~n~ sie po smierci."+
                "~n~Spiesz sie, z czasem bedziesz tracil zycie!",
                new List<Vector3>
                {
        ...
        private void HandleReward(object sender, ElapsedEventArgs e)
        {
            ...
            {
                if (begin && allowSeekerMovementCounter == -100)
                {
                    seeker.AddScore(-5);
                }
                else
                {
                    if (allowSeekerMovementCounter == 0)
                    {
                        BasePlayer.GameTextForAll("~g~Start", 2000, 6);
                        if (seeker != null)
                        {
                            seeker.SetScore(2000);
                            seeker.Position = SpawnPositions[newRandomSpawnPosition];
                            seeker.ToggleControllable(true);
                            allowSeekerMovementCounter = -1;
                        }
                    }
                    else if (allowSeekerMovementCounter > 0)
                    {
                        BasePlayer.GameTextForAll("~r~" + allowSeekerMovementCounter--.ToString(), 999, 6);
                    }
                    else
                    {
                        if (allowSeekerMovementCounter < -20)
                        {
                            foreach (var pl in GameMode.GetPlayers())
                            {
                                pl.SetCheckpoint(new Vector3(-1159.8, -1182.5, 129.2), 10);
                                if (pl != seeker) pl.EnterCheckpoint += Pl_EnterCheckpoint;

                            }
                            allowSeekerMovementCounter = -100;
                        }
                        else
                        {
                            BasePlayer.GameTextForAll("~r~Checkpoint dostepny za "+(20+allowSeekerMovementCounter).ToString(), 999, 6);
                            allowSeekerMovementCounter--;
                        }
                    }
                }
            }
        ...
        protected override void OnStart(List<Player> players)
        {
            ...
            GameMode.Instance.ShowPlayerMarkers(SampSharp.GameMode.Definitions.PlayerMarkersMode.Global);
            GameMode.Instance.ShowNameTags(true);
            GameMode.Instance.SetNameTagDrawDistance(40.0f);
        }
        protected override void Begin(List<Player> players)
        {
            ...
            seeker.Position = new Vector3(pos.X, pos.Y, pos.Z+500);
            seeker.ToggleControllable(false);
            seeker.Disconnected += Seeker_Disconnected;
            seeker.GiveWeapon(SampSharp.GameMode.Definitions.Weapon.M4, 1000);
            //seeker.GiveWeapon(SampSharp.GameMode.Definitions.Weapon.Moltov, 5);
            seeker.GiveWeapon(SampSharp.GameMode.Definitions.Weapon.Sawedoff, 100);
            //seeker.GiveWeapon(SampSharp.GameMode.Definitions.Weapon.FlameThrower, 1000);
            rewardTimer.Start();
        }

        private void Seeker_Disconnected(object sender, SampSharp.GameMode.Events.DisconnectEventArgs e)
        {
            HandleStopGamePlay();
        }
        ...
        {
            if (player==seeker && newScore<=scoreLimit)
            {
                HandleStopGamePlay();
            }
        }
